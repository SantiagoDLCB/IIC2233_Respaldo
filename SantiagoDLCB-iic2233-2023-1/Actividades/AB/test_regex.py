...
class RegexTests(unittest.TestCase):
    ...
    def test_validar_fechas_validas(self):
        ...
        for i, texto in enumerate(casos_validos):
            with self.subTest(i=i):
                respuesta = re.match(
                    self.yolanda.regex_validador_fechas, texto)
    ...
    def test_extraer_signo_valido(self):
        ...
        for i, texto in enumerate(casos_validos):
            with self.subTest(i=i):
                respuesta = re.search(
                    self.yolanda.regex_extractor_signo, texto)
                self.assertIsInstance(respuesta, re.Match)
    ...
    def test_extraer_signo_invalido(self):
        casos_invalidos = [
            # No hay espacio entre Los y signo
            "Los_arianos pueden   dormir la mejor siesta del semestre.",
            # No hay espacio entre signo y pueden
            "Las pisqianas_pueden    comer su poste favorito mañana.",
            # Lis no es Las o Los
            "Lis   tiirinis  pueden   vivir el mejor día de su vida.",
            # Los no parte con mayúscula
            "los\tacuarianas\npueden volver a escuchar la mejor canción de su vida.",
            # Signo no está en plural
            "Los escorpio pueden escuchar música con las aquarianas.",
            # Falta pueden después del signo
            "Las liBRianas serán libres por 1 día.",
